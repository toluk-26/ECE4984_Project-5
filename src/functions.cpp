...
// Connect to the server once an advertised device is found that matches the
// target 48-bit BLE MAC address. This routine will:
// - Connect to the server device (mydevice)
// - Verify that the server has the right service (based on UUID)
// - Verify that the service can provide the temperature characteristic via a
// read and,
//   if so, update the temperature display
// - Verify if the service can provide notifications of the temperature
// characteristic
//   and, if so, register for notifications
// The routine returns false (failure) if any of the verifications fail.
...
bool connectToServer() {
    ...
    // Actually connect to the remote server as found from scanning to have the
    // target MAC address. Exit with error if unsuccessful.
    if (pClient->connect(myDevice)) {
#ifdef DEBUG
        Serial.println(
            "STATUS: Connected to server, but not verified service, etc.");
#endif

        // Set state to show that client is now connected. Update state and
        // display. Delay a bit to allow previous state to be visible.
    ...
    // Obtain a reference to the sensor service at the remote BLE server.
    // Disconnect and fail (return false) if the pointer for the remote service
    // is null.
...
#ifdef DEBUG
    // Display the data source (the name of the server device). Note that the
    // call to myDevice-getName() returns a string of type std::string. The
    // c_str() method will turn this into a C style null-terminated string.
...
#endif
    ///////////////////// TEMPERATURE /////////////////////
    // Obtain a reference to the temperature characteristic in the service of
    // the remote BLE server. Exit with failure if the client cannot find the
    // characteristic.
    ...
    ////////////////// HUMIDITY ///////////////////
    // Obtain a reference to the humidity characteristic in the service of
    // the remote BLE server. Exit with failure if the client cannot find the
    // characteristic.
    ...
    if (pRemoteHumiCharacteristic->canRead()) {
        ...
        char HumiBuffer[8];
        strcpy(HumiBuffer, value.c_str());
        displayHumidityText(HumiBuffer);
